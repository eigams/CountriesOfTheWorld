...
//    [self setGlowEffect:self.backButton];
//    [self setGlowEffect:self.menuButton];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    label.text=[[self.dataController pickerData] objectAtIndex:row];
    return label;
}
...
// |+|                                                                       |+|
// |+|=======================================================================|+|
static NSString *MAP_VIEW_URL = @"http://maps.google.com/maps/api/staticmap?size=%ux%u&&sensor=false&path=color:0x0000ff80|weight:1|fillcolor:0x80FFFF20|%.02f,%.02f|%.02f,%.02f|%.02f,%.02f|%.02f,%.02f|%.02f,%.02f&scale=2";
...
- (void)setSidebarMenuVisibility:(BOOL)hidden {
    
    [self.sideBarMenu hide:hidden];
}
...
- (IBAction)backButtonTapped:(id)sender {
    ...
    [self.sideBarMenu hide:YES];
    [self.navigationController popViewControllerAnimated:YES];
}

- (IBAction)moreButtonTapped:(id)sender {
    
    RKGCountryMoreInfoViewController *vc = (RKGCountryMoreInfoViewController *)[self.storyboard instantiateViewControllerWithIdentifier:@"CountryMoreInfo"];
    [vc performSelector:@selector(setCountry:) withObject:self.country];
